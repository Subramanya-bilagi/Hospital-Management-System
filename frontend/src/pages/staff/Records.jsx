  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        setError(err.response?.data?.message || 'Failed to load system medical records.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-6 max-w-6xl">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
         <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight flex items-center">
            <ClipboardList className="mr-3 text-slate-500 h-6 w-6" /> System Medical Records
         </h1>
       </div>
       ...
       <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 overflow-hidden">
         {loading ? (
             ...
             <div className="py-12 text-center text-slate-500 font-medium">No medical records generated globally yet.</div>
         ) : !error && (
             <div className="space-y-5">
                 {records.map((rec) => (
                    <div key={rec.id} className="border border-slate-100 rounded-2xl p-6 hover:shadow-sm hover:border-slate-200 transition-colors bg-white relative overflow-hidden group">
                       ...
                       <div className="flex flex-col md:flex-row md:items-start justify-between mb-5 pl-2">
                           <div className="flex items-center space-x-3 mb-4 md:mb-0">
                              ...
                              <div className="flex space-x-8">
                                <div>
                                  <p className="text-xs tracking-widest text-slate-400 font-bold uppercase mb-0.5">Patient</p>
                                  <p className="text-[15px] font-bold text-slate-900">{rec.patient_name}</p>
                                </div>
                                <div className="border-l border-slate-200 pl-8">
                                  <p className="text-xs tracking-widest text-slate-400 font-bold uppercase mb-0.5">Authorizing Doctor</p>
                                  <p className="text-sm font-semibold text-slate-500">Dr. {rec.doctor_name}</p>
                                </div>
                              </div>
                           </div>
                           <p className="text-xs font-semibold px-3 py-1 bg-slate-50 border border-slate-100 rounded-md text-slate-500 whitespace-nowrap">
                             {new Date(rec.record_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                           </p>
                       </div>
                       ...
                       <div className="pl-2 pr-4 space-y-4">
                         ...
                         {rec.prescription && (
                             <div>
                               <h4 className="text-[11px] font-bold tracking-wider uppercase text-slate-400 mb-2 mt-4">Prescription details</h4>
                         ...
                         {rec.notes && (
                             <div>
                               <h4 className="text-[11px] font-bold tracking-wider uppercase text-slate-400 mb-2 mt-4">Doctor Notes</h4>
                               <p className="text-sm text-slate-600 bg-slate-50 border border-slate-100 px-4 py-3 rounded-xl italic">
                                 {rec.notes}
                               </p>
                             </div>
                         )}
